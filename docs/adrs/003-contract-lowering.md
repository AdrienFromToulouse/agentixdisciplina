...
|---|---|---|
| `tool.allowlist` (`allowed_tools`) | `tools[]`, `agent_path?` | — |
| `tool.denylist` (`denied_tools`) | `tools[]` | — |
| `tool.call_limit` | `tool?`, `max` | — |
| `tool.no_retry_after_error` | `tool?`, `max_retries` | — |
...
| `order.requires_precondition` (`verify_identity_before_action`) | `action`, `precondition`, `within?` | — |
