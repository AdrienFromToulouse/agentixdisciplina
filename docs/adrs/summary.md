...
- **Episode** — one normalized agent run: turns, tool calls, claims, metrics, coverage, metadata. Decoded from OTLP by a versioned adapter. The unit of evaluation, and the only thing evaluators ever see.
...
- **Verdict** — the result of one evaluator: `pass`, `fail`, `skipped`, or `errored`, classed `deterministic` (blocking) or `probabilistic` (advisory by default).
